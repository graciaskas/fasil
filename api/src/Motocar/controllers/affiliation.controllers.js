		...
		if (!is_exist) {
			...
			if (!affiliation) {
				return res
					.status(400)
					.json({
						message: "An error occured while saving affiliation",
						type: "danger",
					});
			}
			...
			return res
				.status(400)
				.json({ message: "This affiliation already exists !", type: "danger" });
		}
	} catch (error) {
...
exports.updateAffiliation = async (req, res) => {
	try {
		if (!username || !phone || !database)
			throw res.status(400).json("fill all the fields before to continue");
		if (req.user._id !== _id || active === false)
			throw res
				.status(400)
				.json({
					message: "access denied! you're not allowed to perfom such action",
				});
	...
	try {
		...
		if (data) {
			return res
				.status(200)
				.json({
					message: "An affiliation was deleted successfully!",
					type: "success",
				});
		} else {
			return res
				.status(404)
				.json({ message: "affiliation not found!", type: "danger" });
		}
	} catch (error) {
		return res.status(500).json({ message: `${error}`, type: "danger" });
	}
