...
exports.authenticate  = async (req, res, next) => {
    ...
    if(database === null || database === undefined || database == "" )  return res.status(400).json({ 
    ...
    if (!req.headers['authorization'])  return res.status(403).json({ message : 'Unauthorized', type:"danger"});
    try {
