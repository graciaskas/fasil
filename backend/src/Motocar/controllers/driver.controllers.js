...
exports.create = async (req, res, next) => {
	try {
		const { modelIdentification } = req.body; //destructure request body to get moedl
...
exports.find = async (req, res) => {
	...
	try {
		...
		if(q && field) { 
			...
			const data = await Drivers.find(query).sort({ created_at : -1 });			
	...
	try {
		...
		if (engine && owner) {
			...
			if (!driver.isOwner && engine != null) {
				const owner_ = await Owners.find({ engines: engine._id });
				owner = owner_
			}
