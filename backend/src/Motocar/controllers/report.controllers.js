	...
	try {
      ...
      if(filter != null) {
         ...
         ejs.renderFile(
            ...
            { drivers, logo_congo, logo_mairie },  //data
            (err, data) => {
...
exports.identificationReport = async (req, res) => {
	...
	const imageExists = file => { 
		if(!file) { 
			return  "data:image/png;base64,"+no_image;
		}
		if(file === "" || file === null) { 
			return "data:image/png;base64,"+no_image
		}
		...
		return new Promise(async (resolve, reject) => {
			try {
				...
				const owner = owners.filter((owner) =>
					owner.engines.includes(engine._id),
				);
	...
	try {
		...
		return res.json({ message: 'Error: ' + error, type: 'danger' });
	}
};
