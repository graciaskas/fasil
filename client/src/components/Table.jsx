...
const TableCell = ({ tHeads, index, value, className }) => {
   ...
   let class_ = null;
   //check status
   if (cellText == "draft" || cellText == "unassigned")
      class_ = "status bg-info";
   if (cellText == "valid" || cellText == "assigned")
      class_ = "status bg-success";

   if (tHeads[index] == "Etat")
      return (
   ...
   if (cellsImageNames.includes(tHeads[index]))
      return (
         <td key={index} className={className}>
...
export const Table = (props) => {
   ...
   const handleRowClick = (row) => {
      ...
      if (lineRefLinkId) {
         ...
         const stringId = string.slice(string.indexOf(",") + 1, string.length);
         if (viewOnly) return;
