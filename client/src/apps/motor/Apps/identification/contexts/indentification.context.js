...
const Provider = ({ children }) => {
      ...
      try {
         ...
         if (filter) {
            ...
            const { data } = await axios(
               `${URI}/drivers/?q=${value}&field=${filter_}&database=${database}`,
               {
                  method: "GET",
                  headers: {
            ...
            if (value !== "" && value != null && value !== undefined) {
               const search = value.toString().toLowerCase();
               found = drivers.filter((element) =>
                  element.names.includes(search)
               );
               if (search == null || search === "") return getDrivers();
               return setDrivers(found);
            } else {
               ...
               fetch(
                  ...
                  .then((blob) => {
                     const url = window.URL.createObjectURL(
                        new window.Blob([blob], { type: "application/pdf" })
                     );
                     window.open(url);
                     setPrinting(false);
                  })
         ...
         fetch(URI + "/drivers", {
            ...
            .then((data) => {
               ...
               if (data.success) {
                  ...
                  return history.push(
                     "/motor/identifications/?record=" + _id + "&step=motor"
                  ); //update location to engine creation
               }
            })
            .catch((error) => {
               setMessage({ message: "" + error, type: "danger" });
            }); //Fetch catch exceptio
      } catch (error) {
         ...
         fetch(URI + "/engines", {
            ...
            .then((data) => {
               ...
               if (data.success) {
                  ...
                  if (!driver.isOwner) {
                     if (driver.type == 3) {
                        // if is type 3(company delegated)
                        modelOwner.isPP = false; // Set owner model default to is company
                        return history.push(
                           "/motor/identifications/?record=" +
                              _id +
                              "&step=owner&type=company"
                        );
                     }
                     return history.push(
                        "/motor/identifications/?record=" +
                           _id +
                           "&step=owner&type=person"
                     );
                  }
                  return history.push(
                     "/motor/identifications/view/?q=" + driver._id
                  ); //locate identification printing page
               }
            })
            .catch((error) =>
               setMessage({ message: "" + error, type: "danger" })
   ...
   useEffect(() => {
      if (step == 2) {
         setMessage({
            message: "Etap 2 | Enregistrement de l'engin",
            type: "info",
         });
      }
   }, [step]);
