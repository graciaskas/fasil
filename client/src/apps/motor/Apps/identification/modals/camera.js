...
const Camera = (props) => {
   const { display, isOwner, setDisplay, setFileSelected, fileSelected } =
      props;
      ...
      try {
         ...
         setTimeout(() => {
            if (canvas) {
               ...
               setImage(image);
               isOwner
                  ? (modelOwner.image = image)
                  : (modelDriver.image = image);
            }
         }, 5);
      } catch (error) {
         console.error(error);
   ...
   return (
      <React.Fragment>
         <div className="modal " style={{ display }} id="modal_create">
            <div
               className="modal-body w-70 modal-in"
               style={{ padding: "15px" }}
            >
               <div
               ...
               <div className="image-video">
                  ...
                  <div className="image_camera">
                     {displayCanvas ? (
                        <canvas ref={canvas} className="imageCamera"></canvas>
                     ) : (
                        <img src={image} />
                     )}
                     {displayButtons ? (
