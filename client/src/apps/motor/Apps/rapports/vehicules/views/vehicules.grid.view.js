   ...
   const setEngineStolen = async (engine) => {
      ...
      try {
         const response = await fetch(
            `${URI}/engines/${engine}/?update=stolen`,
            {
               method: "PUT",
               headers: {
   ...
   return (
      <div className="container-grids">
         <div className="row">
            {data.map((engine, i) => (
               <div
                  className="col-lg-3 col-md-6 col-sm-6 col-xs-12 box__link"
                  key={i}
               >
                  <div className="box__user">
                     <div className="box__image user">
                        <img
                           src={engine.image ? engine.image : "/src/image.png"}
                           alt=""
                        />
                     </div>
                     <div className="box__description user">
                        <h4>
                           <strong>{engine.marque}</strong> [{engine.model}]{" "}
                        </h4>
                        <p>Plaque : {engine.plaque}</p>
                        <p>{engine.typePneu} Pneu(s)</p>
                        <span className={getStatusColor(engine.stolen)}>
                           {getStolenStatus(engine.stolen)}
                        </span>
