...
const Provider = ({ children }) => {
  ...
  const getGillets = async (setter, type) => {
    try {
      ...
      let reqUrl = type
        ? URI + `/gillets/?database=${database}&type=${type}`
        : URI + `/gillets/?database=${database}`;
      //Request
  ...
  const getAssociations = async () => {
    ...
    try {
      const { data } = await axios.get(
        `${URI}/affiliations/?database=${database}`,
        {
          headers: { Authorization: bearer },
        }
      );
  ...
  const getUtilisateurs = async (setter) => {
    try {
      setLoading(true);
      const { data } = await axios.get(URI + "/users/?&database=" + database, {
        headers: { Authorization: bearer },
      });
      const { message, type, result } = data;
