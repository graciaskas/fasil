...
export const IP_ADDR = window.location.host.replace(":5500", "");
...
const auth = async () => {
   try {
      ...
      if (data) {
         ...
         if (Params.has("selected") && Params.has("id"))
            selected_database = Params.get("selected");
         //If not token => redirect
         if (!token)
            return (window.location.href = `/database/?selected=${selected_database}&id=${database_id}`);
         //Get user info
         const user = { name: "Gracias" };
         if (user.type === "danger") {
            //danger message from server == token
